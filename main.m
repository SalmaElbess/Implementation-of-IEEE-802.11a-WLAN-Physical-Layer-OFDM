...
% check using the data itself
number_of_charachters_errors = sum(data_in ~= data_out)
%% Without AWGN Fixed versus Floating point
out_decoded=[]; out_decoded_fixed = [];
step = 1032*8;
rate= 3/4; mod_type = '16QAM'; estimation_method = 'WE'; M = 16;
for i=1:step:length(data)
    frame = data(i:min(length(data),i+step-1)); 
    % Tansmitter
    tx_frame = WiFi_transmitter(frame, mod_type, rate, Nc, guard_len,'Float');
    tx_frame_fixed = WiFi_transmitter(frame, mod_type, rate, Nc, guard_len,'Fixed');
    % Channel
    Rx_frame = conv(tx_frame,conj(h));
    Rx_frame = Rx_frame(1:end-length(h)+1);
    Rx_frame_fixed = conv(tx_frame_fixed,conj(h));
    Rx_frame_fixed = Rx_frame_fixed(1:end-length(h)+1);
    noise_power = 0;
    data_power = 1;
    % Receiver
    [decoded, ~] = WiFi_receiver(Rx_frame, Nc, guard_len, estimation_method,'Float');
...
% Check
BER = sum(out_decoded ~= data)/length(out_decoded)
BER_fixed = sum(out_decoded_fixed ~= data)/length(out_decoded_fixed)
% Write in file
FileID=fopen('rec_test_file_1.txt','w');
...
step = 1032*8;
snr_dbs = (1:2:25); snrs = 10.^(snr_dbs/10);
...
symbols_WE=[]; symbols_ZF=[]; symbols_recv=[];
    for i=1:step:0.01*length(data)
            ...
            % Receiver 
            [~,rx_syms, rx_syms_WE] = WiFi_receiver(Rx_frame, Nc, guard_len, 'WE','Float');
            [decoded_ZF, rx_syms, rx_syms_ZF] = WiFi_receiver(Rx_frame, Nc, guard_len, 'ZF','Float');
...
title('ZF equalizer');
%% 4.d Comparison between the BER performance of the floating-point and fixed-point 
%%implementations for 16QAM modulation, and 3/4 code rate
  
step = 1032*8;
snr_dbs =(2:2:20); snrs = 10.^(snr_dbs/10);
rate= 3/4; mod_type = "16QAM"; M = 16;

%%fixed point
BERs_WE_fixed = []; BERs_ZF_fixed = []; 
for snr = snrs
out_decoded_WE=[]; out_decoded_ZF=[];
    for i=1:step:length(data)
            frame = data(i:min(length(data),i+step-1)); 
   ...
   BER_WE = sum(out_decoded_WE ~= data)/length(out_decoded_WE);
   BER_ZF = sum(out_decoded_ZF ~= data)/length(out_decoded_ZF);
   BERs_ZF_fixed = cat(2, BERs_ZF_fixed, BER_ZF)
   BERs_WE_fixed = cat(2, BERs_WE_fixed, BER_WE)
end
...
out_decoded_WE=[]; out_decoded_ZF=[];
   ...
   semilogy(snr_dbs, BERs_ZF_fixed,'b-*', snr_dbs, BERs_WE_fixed,'r-*',snr_dbs, BERs_ZF,'g-*', snr_dbs, BERs_WE,'k-*');
   title('BER performance of Floating & Fixed point implementations using ZF & LMMSE equalizers with 16QAM & code rate = 3/4');
   xlabel('E_b/N_0 (dB)'); ylabel('Bit error rate');
   legend('ZF_fixed', 'LMMSE_fixed ','ZF', 'LMMSE ')
   grid on;
